...
#define NOMINMAX
#include <Windows.h>
void MessageMatrix(const MatrixXd& a) {
	std::stringstream s;
	s << "\t";
	...
	MessageBoxA(NULL, s.str().c_str(), "Matrix", NULL);
}
...
MatrixXd EDPFunction::MakeSteps(double qmin, double qmax, double avgqdiff,
	...
	if(_res < 0) {
		// TODO: Change _res to tolerance (epsilon)
	...
	for(std::map<double, double>::iterator iter = steps.begin(); 
		...
		// TODO: How are we going to identify layers
		for(int k = 2; k < nlp; k++)
	...
	finalResults.row(0) = result.row(0).eval();
	...
	for(int i = 1; i < points; i++) {
		...
		if(fabs(result(i, 1) - result(i - 1, 1)) <= 1.0e-6) {
			...
			newsteps++;
			finalResults.row(newsteps) = result.row(i).eval();
		}
	}
...
VectorXd EDPFunction::ComputeParamVector(Model *model, 
...
double EDPFunction::GetUpperLimit() {
	double res = 0.0;
	int nLayers = _params.col(0).size();
