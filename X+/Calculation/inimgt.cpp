...
void ReadParameters(const std::wstring &filename, string obj, paramStruct *p, void* ini) {
	...
	p->params.resize(p->model->GetNumLayerParams());
	...
	for(int i = 0; i < p->model->GetNumLayerParams(); i++) {
		...
		for(int j = 0; j < p->layers; j++) {
			std::string a;
			toEnumString(p->model->GetLayerParamName(i), (j + 1), a);
	...
	p->extraParams.clear();
	for(int i = 0; i < p->model->GetNumExtraParams(); i++) {
		std::string a = p->model->GetExtraParameter(i).name;
...
void WriteParameters(const std::wstring &filename, string obj, paramStruct *p, void* ini) {
	...
	for(int i = 0; i < p->model->GetNumExtraParams(); i++) {
		...
		std::string a = p->model->GetExtraParameter(i).name;
		bool canbeinfinite = p->model->GetExtraParameter(i).canBeInfinite;
