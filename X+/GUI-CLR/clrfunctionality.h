...
static inline void ReadCLRFile(System::String^ fname, vector<double>& x, vector<double>& y) {
		...
		cli::array<wchar_t>^ arr = fname->ToCharArray();
...
static inline void clrToString(System::String^ str, std::string& result) {
	    cli::array<wchar_t>^ arr = str->ToCharArray();
...
template<typename T> static std::vector<T> arraytovector(cli::array<T>^ arr) {
...
template<typename T> static std::vector<T> ^vectortoarray(const std::vector<T> &vec) {
