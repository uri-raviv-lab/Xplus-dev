...
namespace GUICLR {
	...
	void FormFactor::AddCaille(double eta, int h) {
		caillePeaksListView->Items->Add(h.ToString());
		caillePeaksListView->Items[caillePeaksListView->Items->Count - 1]->SubItems->Add(eta.ToString());
	}

	void FormFactor::AddPeak(double amp, char ampMut, double fwhm, char fwhmMut, double center, char centerMut) {
		ListViewItem ^lvi;
		listView_peaks->Items->Add("Peak " + (listView_peaks->Items->Count + 1).ToString());
		lvi = listView_peaks->Items[listView_peaks->Items->Count - 1];

		lvi->SubItems->Add(amp.ToString("0.000000"));
		lvi->SubItems->Add(gcnew System::String(ampMut, 1));
		lvi->SubItems->Add(fwhm.ToString("0.000000"));
		lvi->SubItems->Add(gcnew System::String(fwhmMut, 1));
		lvi->SubItems->Add(center.ToString("0.000000"));
		lvi->SubItems->Add(gcnew System::String(centerMut, 1));
	...
	**/
	...
	void FormFactor::listView_peaks_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {
		if(listView_peaks->SelectedIndices->Count == 0) {
			// No Items
			label14->Text = "<None>";
			removePeak->Enabled = false;
			
			ampBox->Enabled = false;
			fwhmBox->Enabled = false;
			centerBox->Enabled = false;
			ampTrack->Value = int((ampTrack->Minimum + ampTrack->Maximum) / 2.0);
			ampTrack->Enabled = false;
			fwhmTrack->Value = int((fwhmTrack->Minimum + fwhmTrack->Maximum) / 2.0);
			fwhmTrack->Enabled = false;
			centerTrack->Value = int((centerTrack->Minimum + centerTrack->Maximum) / 2.0);
			centerTrack->Enabled = false;
			ampMut->Enabled = false;
			fwhmMut->Enabled = false;
			centerMut->Enabled = false;

			if(wgtFit && wgtFit->graph) {
				wgtFit->graph->Deselect();
				wgtFit->Invalidate();
			}

		} else if(listView_peaks->SelectedIndices->Count > 1) {
			// Multiple Items
			label14->Text = "<Multiple>";
			ampBox->Enabled = false;
			fwhmBox->Enabled = false;
			centerBox->Enabled = false;
			if(!_bGenerateModel) { 
				ampMut->Enabled = true;
				fwhmMut->Enabled = true;
				centerMut->Enabled = true;
			}
			...
			removePeak->Enabled = true;
			for(int i = 0; i < listView_peaks->SelectedIndices->Count; i++) {
				if(listView_peaks->Items[listView_peaks->SelectedIndices[i]]->SubItems[2]->Text->Equals("-"))
					ampMut->Enabled = false;
				if(listView_peaks->Items[listView_peaks->SelectedIndices[i]]->SubItems[4]->Text->Equals("-"))
					fwhmMut->Enabled = false;
				if(listView_peaks->Items[listView_peaks->SelectedIndices[i]]->SubItems[6]->Text->Equals("-"))
					centerMut->Enabled = false;
			...
			// One Item
			ListViewItem ^lvi = listView_peaks->SelectedItems[0];
			label14->Text = lvi->Text;
		
			removePeak->Enabled = true;

			ampBox->Enabled = true;
			fwhmBox->Enabled = true;
			centerBox->Enabled = true;
			ampTrack->Enabled = true;
			fwhmTrack->Enabled = true;
			centerTrack->Enabled = true;
			if(!_bGenerateModel) {
				ampMut->Enabled = true;
				fwhmMut->Enabled = true;
				centerMut->Enabled = true;
			}
			if(lvi->SubItems[1]->Text->Equals("N/A")) {
				ampBox->Enabled = false;
				ampTrack->Enabled = false;
			}
			if(lvi->SubItems[2]->Text->Equals("-"))
				ampMut->Enabled = false;
			if(lvi->SubItems[3]->Text->Equals("N/A")) {
				fwhmBox->Enabled = false;
				fwhmTrack->Enabled = false;
			}
			if(lvi->SubItems[4]->Text->Equals("-"))
				fwhmMut->Enabled = false;
			if(lvi->SubItems[5]->Text->Equals("N/A")) {
				centerBox->Enabled = false;
				centerTrack->Enabled = false;
			}
			...
			ampBox->Text = lvi->SubItems[1]->Text;
			fwhmBox->Text = lvi->SubItems[3]->Text;
			centerBox->Text = lvi->SubItems[5]->Text;

			ampMut->Checked = (lvi->SubItems[2]->Text->Equals("Y") ? true : false);
			fwhmMut->Checked = (lvi->SubItems[4]->Text->Equals("Y") ? true : false);
			centerMut->Checked = (lvi->SubItems[6]->Text->Equals("Y") ? true : false);
			...
			if(wgtFit && wgtFit->graph) {
				...
				int srt = -1, nd = -1;
				double lft, rgt;
				lft = clrToDouble(listView_peaks->Items[listView_peaks->SelectedIndices[0]]->SubItems[5]->Text) - ((5.0 / ((double)peakfit->SelectedIndex + 2) * clrToDouble(listView_peaks->Items[listView_peaks->SelectedIndices[0]]->SubItems[3]->Text)));
				rgt = clrToDouble(listView_peaks->Items[listView_peaks->SelectedIndices[0]]->SubItems[5]->Text) + ((5.0 / ((double)peakfit->SelectedIndex + 2) * clrToDouble(listView_peaks->Items[listView_peaks->SelectedIndices[0]]->SubItems[3]->Text)));
				for (int hrt = 1; hrt < (int)wgtFit->graph->x->size(); hrt++) {
					if (srt < 0 && wgtFit->graph->x->at(hrt) > lft)
						srt = hrt - 1;
					if (wgtFit->graph->x->at(hrt) > rgt) {
	...
	void FormFactor::ampMut_Click(System::Object^  sender, System::EventArgs^  e) {
		for(int i  = 0; i < listView_peaks->SelectedItems->Count; i++)
				listView_peaks->SelectedItems[i]->SubItems[2]->Text = ampMut->Checked ? "Y" : "N";
		save->Enabled = true;
	}

	void FormFactor::fwhmMut_Click(System::Object^  sender, System::EventArgs^  e) {
		for(int i  = 0; i < listView_peaks->SelectedItems->Count; i++)
				listView_peaks->SelectedItems[i]->SubItems[4]->Text = fwhmMut->Checked ? "Y" : "N";
		save->Enabled = true;
	}
	
	void FormFactor::centerMut_Click(System::Object^  sender, System::EventArgs^  e) {
		for(int i  = 0; i < listView_peaks->SelectedItems->Count; i++)
				listView_peaks->SelectedItems[i]->SubItems[6]->Text = centerMut->Checked ? "Y" : "N";
		save->Enabled = true;
	}

	void FormFactor::StructureFactor_ParamTextLeave(System::Object^  sender, System::EventArgs^  e) {
		double res;
		std::string str;
		char f[128] = {0};
		...
		if(sender == ampBox)
			listView_peaks->SelectedItems[0]->SubItems[1]->Text = ampBox->Text;
		if(sender == fwhmBox)
			listView_peaks->SelectedItems[0]->SubItems[3]->Text = fwhmBox->Text;
		if(sender == centerBox)
			listView_peaks->SelectedItems[0]->SubItems[5]->Text = centerBox->Text;

		if(sender == cailleValueTextBox)
			cailleParamListView->SelectedItems[0]->SubItems[1]->Text = cailleValueTextBox->Text;
		if(sender == cailleMinTextBox)
			cailleParamListView->SelectedItems[0]->SubItems[2]->Text = cailleMinTextBox->Text;
		if(sender == cailleMaxTextBox && cailleParamListView->SelectedItems->Count > 0)
			cailleParamListView->SelectedItems[0]->SubItems[3]->Text = cailleMaxTextBox->Text;
					
		SFparamErrors->clear();
		SFmodelErrors->clear();

		//For peak highlighting purposes
		listView_peaks_SelectedIndexChanged(sender, e);

		if(liveRefreshToolStripMenuItem->Checked &&
				!(sender == cailleMaxTextBox || sender == cailleMinTextBox))
			UpdateGraph();
	...
	void FormFactor::cailleParamListView_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {
		if(cailleParamListView->SelectedIndices->Count == 0) {
			// No Items
			cailleValueTrackBar->Value = int((cailleValueTrackBar->Minimum + cailleValueTrackBar->Maximum) / 2.0);
			cailleValueGroupbox->Enabled = false;
			cailleMinGroupbox->Enabled = false;
			cailleMaxGroupbox->Enabled = false;
			...
			// One Item
			ListViewItem ^lvi = cailleParamListView->SelectedItems[0];
		
			cailleValueGroupbox->Enabled = true;
			cailleValueTextBox->Enabled = true;
			cailleValueTrackBar->Enabled = true;
			cailleMinGroupbox->Enabled = true;
			cailleMinTextBox->Enabled = true;
			cailleMaxGroupbox->Enabled = true;
			cailleMaxTextBox->Enabled = true;

			cailleValueTextBox->Text = lvi->SubItems[1]->Text;
			cailleMinTextBox->Text = lvi->SubItems[2]->Text;
			cailleMaxTextBox->Text = lvi->SubItems[3]->Text;
		}
	}
	...
	void FormFactor::wgtFit_MouseDown(System::Object^  sender, System::Windows::Forms::MouseEventArgs^  e) {
		...
		if(tabControl1->SelectedIndex == 1 && !_bMasking) {
			if(cailleGroupbox->Visible == true || !sfUseCheckBox->Checked) {
				_pressY = -1.0;
				_pressX = -1.0;
				return;
			}
	...
	void FormFactor::wgtFit_MouseUp(System::Object^  sender, System::Windows::Forms::MouseEventArgs^  e) {
		...
		if(tabControl1->SelectedIndex == 1 && !_bMasking) {
			if(!wgtFit || !wgtFit->graph  || cailleGroupbox->Visible == true || !(sfUseCheckBox->Checked))
				 return;
				...
				switch(GetPeakType()){
					...
					case SHAPE_GAUSSIAN:
						if(sigmaToolStripMenuItem->Checked)
							AddPeak(fabs(deltaY) * (0.5 * fw * sqrt(PI / log(2.0))), 'Y', fw / 2.355, 'Y', _pressX, 'Y');
						else
							AddPeak(fabs(deltaY) * (0.5 * fw * sqrt(PI / log(2.0))), 'Y', fw, 'Y', _pressX, 'Y');
						break;
			...
			// TODO: crop _mask with the other _vectors
	...
	// Copies the peak parameters from listView_Peaks to *peaks
	void FormFactor::GetPeaksFromListView(peakStruct *peaks) {
		for(int i = 0; i < listView_peaks->Items->Count; i++) {
			peaks->amp.push_back(clrToDouble(listView_peaks->Items[i]->SubItems[1]->Text));
			peaks->fwhm.push_back(clrToDouble(listView_peaks->Items[i]->SubItems[3]->Text));
			peaks->center.push_back(clrToDouble(listView_peaks->Items[i]->SubItems[5]->Text));

			peaks->amp.back().mut = (char(listView_peaks->Items[i]->SubItems[2]->Text[0]));
			peaks->fwhm.back().mut = (char(listView_peaks->Items[i]->SubItems[4]->Text[0]));
			peaks->center.back().mut = (char(listView_peaks->Items[i]->SubItems[6]->Text[0]));
		}	
	}

	
	// Copies the *peaks to listView_peaks
	void FormFactor::SetPeaks(peakStruct *peaks) {
		for(int i = 0; i < listView_peaks->Items->Count; i++) {
			listView_peaks->Items[i]->SubItems[1]->Text = peaks->amp[i].value.ToString("0.000000");
			listView_peaks->Items[i]->SubItems[3]->Text = peaks->fwhm[i].value.ToString("0.000000");
			listView_peaks->Items[i]->SubItems[5]->Text = peaks->center[i].value.ToString("0.000000");
	...
	void FormFactor::scrollerTimer_Tick(System::Object^  sender, System::EventArgs^  e) {
		...
		double mid = (ampTrack->Maximum - ampTrack->Minimum) / 2.0;
		if(tabControl1->SelectedIndex == 1) { // In SF tab
			for(int i = 0; i < listView_peaks->SelectedItems->Count; i++) {
				if(ampTrack->Value != mid) {
					dealWithTrackBar(listView_peaks->SelectedItems[i], 1, ampTrack, ampBox, 
						50.0, absDesc);
					bChanged = true;
				}

				mid = (fwhmTrack->Maximum - fwhmTrack->Minimum) / 2.0;
				if(fwhmTrack->Value != mid) {
					dealWithTrackBar(listView_peaks->SelectedItems[i], 3, fwhmTrack, fwhmBox, 
						50.0, absDesc);
					bChanged = true;
				}

				mid = (centerTrack->Maximum - centerTrack->Minimum) / 2.0;
				if(centerTrack->Value != mid) {
					dealWithTrackBar(listView_peaks->SelectedItems[i], 5, centerTrack, centerBox, 
						75.0, absDesc);
					bChanged = true;
				}
			} // end for listView_peaks

			mid = (cailleValueTrackBar->Maximum - cailleValueTrackBar->Minimum) / 2.0;
			if(cailleValueTrackBar->Value != mid) {
				dealWithTrackBar(cailleParamListView->SelectedItems[0], 1, cailleValueTrackBar, cailleValueTextBox, 
					10.0, absDesc);
					bChanged = true;
			}
			...
			if(GroupBoxList) {
				for(int j = 0; j < GroupBoxList->Count; j++) {
					TrackBar ^tb = GroupBoxList[j]->track;
					TextBox ^tex = GroupBoxList[j]->text;
					mid = (tb->Maximum - tb->Minimum) / 2.0;

					for(int i = 0; i < listViewFF->SelectedItems->Count; i++) {
						if(tb->Value != mid){
							dealWithTrackBar(listViewFF->SelectedItems[i], 
								GroupBoxList[j]->rValue->Checked
								? LV_VALUE(j)
								: LV_SIGMA(j, _model->GetNumLayerParams()),
								tb, tex, 
								75.0, absDesc);						
			...
			if(exParamGroupbox->track->Value != mid) {
				dealWithTrackBar(listView_Extraparams->Items[paramBox->SelectedIndex],
					...
					75.0, _model->GetExtraParameter(paramBox->SelectedIndex));
			...
			/*
			mid = (PDtrackBar->Maximum - PDtrackBar->Minimum) / 2.0;
			if(PDtrackBar->Value != mid) {
				String ^str = PolytextBox->Text;
				int G = str->LastIndexOf(".");
				double factor = 75.0;
				double val = clrToDouble((G >= 0) ? str->Remove(G, 1) : str);
				if(fabs(val / factor) <= 1.0)
					factor = val - 0.9 * (val < 0.0 ? -1.0 : 1.0);
				double newVal = clrToDouble(PolytextBox->Text);
				if(fabs(val) < 1.1)
					newVal += (newVal < 0.0 ? -1.0 : 1.0) * (1.0e-6);
				newVal *= (1.0 + (newVal < 0.0 ? -1.0 : 1.0) * ((PDtrackBar->Value - mid) / PDtrackBar->Maximum / fabs(factor)));
				if(newVal < 0.0)
					newVal = 0.0;
				PolytextBox->Text = newVal.ToString("0.000000");
				//setPolySigma(newVal); <-- TODO
				save->Enabled = true;
	...
	void FormFactor::Move2Phases_Click(System::Object^  sender, System::EventArgs^  e) {
		...
		if(listView_peaks->SelectedIndices->Count > 0) {
			...
			Caille_button->Enabled = (wgtFit && wgtFit->graph);
		}
		...
		// Generates the phase's peaks. TODO: Cleanup
	...
	void FormFactor::fitphase_Click(System::Object^  sender, System::EventArgs^  e) {
		...
		if(sender == undoPhases) {
			...
			// Generates the last phase's peaks. TODO: Cleanup
			std::vector<std::string> indices_locs;
		...
		*/
		...
		while (unsigned(cnt + first_pos) < wgtFit->graph->x[1].size()) {
			for (cnt = 1; cnt <= segment; cnt++) {
				...
				sum += val;
				// TODO: Check if seqment is mostly noise (delta-correlation)
	...
	void FormFactor::phaseorder_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e){	
		...
		// If applicable, show the Caille button
		if(_model->HasSpecializedSF()) {
			if(order->SelectedIndex == 1)
				Caille_button->Visible = true;
			else
	...
	void FormFactor::listView_phases_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {
	 ...
	 void FormFactor::AddCailleParam(System::String ^str, ParamMode mode, double defaultValue) {
		 System::String ^ valuestr;

		if(mode == MODE_PRECISION)
			valuestr = defaultValue.ToString("0.000000000000");
		else if(mode == MODE_INTEGER)
			valuestr = defaultValue.ToString("0.");
		else
			valuestr = defaultValue.ToString("0.000000");

		cailleParamListView->Items->Add(str);															// Parameter
		cailleParamListView->Items[cailleParamListView->Items->Count - 1]->SubItems->Add(valuestr);		// Value
		cailleParamListView->Items[cailleParamListView->Items->Count - 1]->SubItems->Add("0.000000");	// Min.
		cailleParamListView->Items[cailleParamListView->Items->Count - 1]->SubItems->Add("0.000000");	// Max.
...
//		phase->ummax	 = clrToDouble(listView_phases->Items[6]->SubItems[5]->Text);
		...
		if(dim >= 2) {
			phase->bM         = (char)listView_phases->Items[1]->SubItems[2]->Text[0];
			phase->gammaM     = (char)listView_phases->Items[2]->SubItems[2]->Text[0];
			if(dim >= 3) {
				phase->cM     = (char)listView_phases->Items[3]->SubItems[2]->Text[0];
				phase->alphaM = (char)listView_phases->Items[4]->SubItems[2]->Text[0];
				phase->betaM  = (char)listView_phases->Items[5]->SubItems[2]->Text[0];
			} else
				phase->cM = phase->alphaM = phase->betaM = false;
		} else
			phase->bM = phase->gammaM = false;
...
//		phase->umM    = (int)clrToDouble(listView_phases->Items[6]->SubItems[7]->Text) == 1 ? 'Y': 'N';
	...
	void FormFactor::removeCaille_Click(System::Object^  sender, System::EventArgs^  e) {
	//	while(caillePeaksListView->SelectedItems->Count > 0)
	//		caillePeaksListView->Items->Remove(caillePeaksListView->SelectedItems[0]);

	//	save->Enabled = true;
	}

	void FormFactor::caillePeaksListView_KeyDown(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) {
	//	//If the user selected indices and pressed delete/backspace, remove indicies
	//	if(e->KeyCode == Keys::Delete || e->KeyCode == Keys::Back) {
	//		while(caillePeaksListView->SelectedItems->Count > 0)
	//			removeCaille_Click(this, e);
	//	}
	}

	void FormFactor::caillePeaksListView_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e) {
		//if(caillePeaksListView->SelectedItems->Count == 0) {
		//	removeCaille->Enabled = false;
		//	hTextBox->Enabled = false;
		//} else {
		//	removeCaille->Enabled = true;
		//	hTextBox->Text = clrToDouble(caillePeaksListView->SelectedItems[0]->SubItems[1]->Text).ToString();
		//	hTextBox->Enabled = true;
		//}
	}

	void FormFactor::GetCailleFromGUI(graphTable *Caille, cailleParamStruct *cailleP) {
		Caille->x.clear();
		Caille->y.clear();

		for(int i = 0; i < caillePeaksListView->Items->Count; i++) {
			Caille->x.push_back(int(clrToDouble(caillePeaksListView->Items[i]->SubItems[0]->Text)));
			Caille->y.push_back(clrToDouble(caillePeaksListView->Items[i]->SubItems[1]->Text));
		}

		cailleP->amp.clear();
		cailleP->amp.resize(1);
		cailleP->eta.clear();
		cailleP->eta.resize(1);
		cailleP->N.clear();
		cailleP->N.resize(1);
		cailleP->sig.clear();
		cailleP->sig.resize(1);
		cailleP->N_diffused.clear();
		cailleP->N_diffused.resize(1);
	
		cailleP->amp.at(0).value = clrToDouble(cailleParamListView->Items[0]->SubItems[1]->Text);
		cailleP->amp.at(0).min	 = clrToDouble(cailleParamListView->Items[0]->SubItems[2]->Text);
		cailleP->amp.at(0).max	 = clrToDouble(cailleParamListView->Items[0]->SubItems[3]->Text);
		cailleP->amp.at(0).mut	 = cailleAmpCheckbox->Checked ? 'Y' : 'N';

		cailleP->eta.at(0).value = clrToDouble(cailleParamListView->Items[1]->SubItems[1]->Text);
		cailleP->eta.at(0).min	 = clrToDouble(cailleParamListView->Items[1]->SubItems[2]->Text);
		cailleP->eta.at(0).max	 = clrToDouble(cailleParamListView->Items[1]->SubItems[3]->Text);
		cailleP->eta.at(0).mut	 = cailleEtaCheckbox->Checked ? 'Y' : 'N';

		cailleP->N.at(0).value	 = clrToDouble(cailleParamListView->Items[2]->SubItems[1]->Text);
		cailleP->N.at(0).min	 = clrToDouble(cailleParamListView->Items[2]->SubItems[2]->Text);
		cailleP->N.at(0).max	 = clrToDouble(cailleParamListView->Items[2]->SubItems[3]->Text);
		cailleP->N.at(0).mut	 = cailleNCheckbox->Checked ? 'Y' : 'N';

		cailleP->sig.at(0).value = clrToDouble(cailleParamListView->Items[3]->SubItems[1]->Text);
		cailleP->sig.at(0).min	 = clrToDouble(cailleParamListView->Items[3]->SubItems[2]->Text);
		cailleP->sig.at(0).max	 = clrToDouble(cailleParamListView->Items[3]->SubItems[3]->Text);
		cailleP->sig.at(0).mut	 = cailleSigCheckbox->Checked ? 'Y' : 'N';

		cailleP->N_diffused.at(0).value = clrToDouble(cailleParamListView->Items[4]->SubItems[1]->Text);
		cailleP->N_diffused.at(0).min	 = clrToDouble(cailleParamListView->Items[4]->SubItems[2]->Text);
		cailleP->N_diffused.at(0).max	 = clrToDouble(cailleParamListView->Items[4]->SubItems[3]->Text);
		cailleP->N_diffused.at(0).mut	 = cailleN_diffusedCheckBox->Checked ? 'Y' : 'N';
	}


	void FormFactor::SetCailletoGUI(graphTable *Cailles, cailleParamStruct *Caillesdrawing) {
		// Ensure that Caillesdrawing is not empty
		if(Caillesdrawing->amp.size() < 1 || 
				Caillesdrawing->eta.size() < 1 || 
				Caillesdrawing->N.size() < 1 || 
				Caillesdrawing->sig.size() < 1)
			return;

		caillePeaksListView->Items->Clear();
		if(Caillesdrawing->N_diffused.size() < 1)
			Caillesdrawing->N_diffused.push_back(0.0);

		for(unsigned int i = 0; i < Cailles->x.size(); i++)
			AddCaille(Cailles->y[i], int(Cailles->x[i]));

		// Only change the fields form the default if there are better parameters
		if(!(Caillesdrawing->amp.at(0).value < 1.0e-8) &&
				!(Caillesdrawing->N.at(0).value < 1.0e-8) ) {

			ListViewItem ^lvi = cailleParamListView->Items[0];
			lvi->SubItems[1]->Text = (Caillesdrawing->amp.at(0).value).ToString("0.000000");
			lvi->SubItems[2]->Text = (Caillesdrawing->amp.at(0).min).ToString("0.000000");
			lvi->SubItems[3]->Text = (Caillesdrawing->amp.at(0).max).ToString("0.000000");
			cailleAmpCheckbox->Checked = (Caillesdrawing->amp.at(0).mut == 'Y');

			lvi = cailleParamListView->Items[1];
			lvi->SubItems[1]->Text = (Caillesdrawing->eta.at(0).value).ToString("0.000000");
			lvi->SubItems[2]->Text = (Caillesdrawing->eta.at(0).min).ToString("0.000000");
			lvi->SubItems[3]->Text = (Caillesdrawing->eta.at(0).max).ToString("0.000000");
			cailleEtaCheckbox->Checked = (Caillesdrawing->eta.at(0).mut == 'Y');

			lvi = cailleParamListView->Items[2];
			lvi->SubItems[1]->Text = (Caillesdrawing->N.at(0).value).ToString("0.000000");
			lvi->SubItems[2]->Text = (Caillesdrawing->N.at(0).min).ToString("0.000000");
			lvi->SubItems[3]->Text = (Caillesdrawing->N.at(0).max).ToString("0.000000");
			cailleNCheckbox->Checked = (Caillesdrawing->N.at(0).mut == 'Y');

			lvi = cailleParamListView->Items[3];
			lvi->SubItems[1]->Text = (Caillesdrawing->sig.at(0).value).ToString("0.000000");
			lvi->SubItems[2]->Text = (Caillesdrawing->sig.at(0).min).ToString("0.000000");
			lvi->SubItems[3]->Text = (Caillesdrawing->sig.at(0).max).ToString("0.000000");
			cailleSigCheckbox->Checked = (Caillesdrawing->sig.at(0).mut == 'Y');

			lvi = cailleParamListView->Items[4];
			lvi->SubItems[1]->Text = (Caillesdrawing->N_diffused.at(0).value).ToString("0.000000");
			lvi->SubItems[2]->Text = (Caillesdrawing->N_diffused.at(0).min).ToString("0.000000");
			lvi->SubItems[3]->Text = (Caillesdrawing->N_diffused.at(0).max).ToString("0.000000");
			cailleN_diffusedCheckBox->Checked = (Caillesdrawing->N_diffused.at(0).mut == 'Y');
		}
	}
	...
	**/
	...
	void FormFactor::PhasesCompleted () {
		...
		phaseStruct p;
		if(!_curPhases) GetPhasesFromListView(&p);
		else p = *_curPhases;
	...
	void FormFactor::Caille_button_Click(System::Object^  sender, System::EventArgs^  e) {
		/*
		Hide the regular peak group box / show Caille groupbox
		Transfer indices and peak positions from table peak positions to Caille listview
		Redesign Caille groupbox to include relevant parameters
		Replace SF with Caille graph
		Make sure that recalculating FF/SF/BG takes Caille into account and not other SF
		*/
		cailleGroupbox->Enabled = true;
		cailleGroupbox->Visible = true;
		SetPeakType(SHAPE_CAILLE);
		...
		if(listView_PeakPosition->Items->Count > 0) {
			...
			int j = -1;
			for(int i = 0; i < listView_PeakPosition->Items->Count; i++) {
				if(clrToDouble(listView_PeakPosition->Items[i]->SubItems[0]->Text) > 0.0 && listView_PeakPosition->Items[i]->SubItems->Count > 1) { //Avi: this is stupid, we shouldn't have different lengths of subitems
					System::String ^str = listView_PeakPosition->Items[i]->SubItems[1]->Text->Substring(1);
					str->Remove(str->Length - 1, 1);
					caillePeaksListView->Items->Add(((int)clrToDouble(str)).ToString());
					j++;

				}
				else
					continue;
				caillePeaksListView->Items[j]->SubItems->Add(listView_PeakPosition->Items[i]->SubItems[0]->Text);
			}
		}
	}

	void FormFactor::returnToLineshapeButton_Click(System::Object^  sender, System::EventArgs^  e) {
		cailleGroupbox->Visible = false;
		cailleGroupbox->Enabled = true;
		peakfit_SelectedIndexChanged(sender, e);
	...
	/**
	 * Calculate the relevant reciprical vectors
	**/
	...
	void FormFactor::showCailleButton_Click(System::Object^  sender, System::EventArgs^  e) {
		cailleGroupbox->Enabled = true;
		cailleGroupbox->Visible = true;
		SetPeakType(SHAPE_CAILLE);
		Peakfitter->Visible = false;
	...
	void FormFactor::SetCaillePeakstoGUI(peakStruct *p) {
		if( cailleParamListView->Items->Count == 0) return;
		cailleParamListView->Items[0]->SubItems[1]->Text = p->amp.at(0).value.ToString("0.000000");
		cailleParamListView->Items[1]->SubItems[1]->Text = p->center.at(0).value.ToString("0.000000");
		cailleParamListView->Items[2]->SubItems[1]->Text = p->fwhm.at(0).value.ToString("0.000000");
		cailleParamListView->Items[3]->SubItems[1]->Text = p->cailleSigma.at(0).value.ToString("0.000000");
		cailleParamListView->Items[4]->SubItems[1]->Text = p->cailleNDiffused.at(0).value.ToString("0.000000");

		cailleAmpCheckbox->Checked			= p->amp.at(0).mut == 'Y';
		cailleEtaCheckbox->Checked			= p->center.at(0).mut == 'Y';
		cailleNCheckbox->Checked			= p->fwhm.at(0).mut == 'Y';
		cailleSigCheckbox->Checked			= p->cailleSigma.at(0).mut == 'Y';
		cailleN_diffusedCheckBox->Checked	= p->cailleNDiffused.at(0).mut == 'Y';
	}

	void FormFactor::GetCaillePeaksFromGUI(peakStruct *p) {
		if( cailleParamListView->Items->Count == 0) return;
		p->amp.push_back(clrToDouble(cailleParamListView->Items[0]->SubItems[1]->Text));
		p->amp.back().mut = (cailleAmpCheckbox->Checked ? 'Y' : 'N');
		p->amp.back().min = clrToDouble(cailleParamListView->Items[0]->SubItems[2]->Text);
		p->amp.back().max = clrToDouble(cailleParamListView->Items[0]->SubItems[3]->Text);

		p->center.push_back(clrToDouble(cailleParamListView->Items[1]->SubItems[1]->Text));
		p->center.back().mut = (cailleEtaCheckbox->Checked ? 'Y' : 'N');
		p->center.back().min = clrToDouble(cailleParamListView->Items[1]->SubItems[2]->Text);
		p->center.back().max = clrToDouble(cailleParamListView->Items[1]->SubItems[3]->Text);
		
		p->fwhm.push_back(clrToDouble(cailleParamListView->Items[2]->SubItems[1]->Text));
		p->fwhm.back().mut = (cailleNCheckbox->Checked ? 'Y' : 'N');
		p->fwhm.back().min = clrToDouble(cailleParamListView->Items[2]->SubItems[2]->Text);
		p->fwhm.back().max = clrToDouble(cailleParamListView->Items[2]->SubItems[3]->Text);

		p->cailleSigma.push_back(clrToDouble(cailleParamListView->Items[3]->SubItems[1]->Text));
		p->cailleSigma.back().mut = (cailleSigCheckbox->Checked ? 'Y' : 'N');
		p->cailleSigma.back().min = clrToDouble(cailleParamListView->Items[3]->SubItems[2]->Text);
		p->cailleSigma.back().max = clrToDouble(cailleParamListView->Items[3]->SubItems[3]->Text);

		p->cailleNDiffused.push_back(clrToDouble(cailleParamListView->Items[4]->SubItems[1]->Text));
		p->cailleNDiffused.back().mut = (cailleN_diffusedCheckBox->Checked ? 'Y' : 'N');
		p->cailleNDiffused.back().min = clrToDouble(cailleParamListView->Items[4]->SubItems[2]->Text);
		p->cailleNDiffused.back().max = clrToDouble(cailleParamListView->Items[4]->SubItems[3]->Text);
	}
	...
	void FormFactor::sigmaToolStripMenuItem_CheckedChanged(System::Object^  sender, System::EventArgs^  e) {
		...
		SetSigma(_bsigma = sigmaToolStripMenuItem->Checked);
		if (_bsigma && GetPeakType() == SHAPE_GAUSSIAN) listView_peaks->Columns[3]->Text = UnicodeChars::sigmaUnicode;
		else listView_peaks->Columns[3]->Text =  L"FWHM";
		label12->Text = listView_peaks->Columns[3]->Text;
		if(liveRefreshToolStripMenuItem->Checked && sender != nullptr)
			UpdateGraph();
