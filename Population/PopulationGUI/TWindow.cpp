...
namespace PopulationGUI {
	...
	void TWindow::TWindow_Load(System::Object ^sender, System::EventArgs ^e) {
		...
		setAccuracySettings(false, false, false, true);
	...
	void TWindow::useLogFittingToolStripMenuItem_CheckedChanged(System::Object ^sender, System::EventArgs ^e) {
		setAccuracySettings(true, true, true, useLogFittingToolStripMenuItem->Checked);
	}
...
#pragma endregion
	...
	//
	void TWindow::LoadFile(System::Object ^sender) {
		FileExpressionPanel^ par = GetParent(sender);
		...
		if(ofd->ShowDialog() == System::Windows::Forms::DialogResult::Cancel)
			return;
		std::wstring file = clrToWstring(ofd->FileName);
	...
	bool TWindow::modifyGraph(FileExpressionPanel ^ grr, bool ex) {
		...
		if(wgtGraph && x->size() > 1 && y->size() > 1) {
			if(grr->graphIndex < 0) {
				if(wgtGraph->graph) {
					wgtGraph->graph->Add(grr->GetColorRef(), DRAW_LINES, *x, *y);
				} else {
					RECT area;
					area.top = 0;
					area.left = 0;
					...
					this->wgtGraph->graph = gcnew Graph(area,
										grr->GetColorRef(), 
										DRAW_LINES, *x, *y,
										logQcheckBox->Checked/*logQ*/,
										logIcheckBox->Checked/*logI*/);
					wgtGraph->graph->SetXLabel(" [nm" + GUICLR::UnicodeChars::minusOne + "]");
					wgtGraph->graph->SetYLabel("Intensity [a.u.]");
				}
				grr->graphIndex = wgtGraph->graph->numGraphs - 1;

			}  else {
				// Modify existing
				wgtGraph->graph->Modify(grr->graphIndex, *x, *y);
			}
	...
	void TWindow::visCheckedChanged(System::Object^  sender, System::EventArgs^  e) {
		FileExpressionPanel^ par = GetParent(sender);
		wgtGraph->graph->SetGraphVisibility(par->graphIndex, par->visCheckBox->Checked);
	...
	void TWindow::ChangeColor(System::Object ^sender, System::EventArgs ^e) {
		...
		if(wgtGraph && wgtGraph->graph) {
			wgtGraph->graph->ChangeColor(par->graphIndex, par->GetColorRef());
			wgtGraph->graph->Modify(par->graphIndex,
				wgtGraph->graph->x[par->graphIndex],
				wgtGraph->graph->y[par->graphIndex]);
			draw();
		}
	}
	...
	void TWindow::logCheckedChanged(System::Object ^sender, System::EventArgs ^e) {
		...
		Q = logQcheckBox->Checked ? SCALE_LOG : SCALE_LIN;
		I = logIcheckBox->Checked ? SCALE_LOG : SCALE_LIN;

		if(wgtGraph  && wgtGraph->graph) {
			wgtGraph->graph->SetScale(I, Q);
			draw();
		}
	}
...
//		Eigen::Hyperplane<double, > hyp(
	...
	void TWindow::fitNowButton_Click(System::Object ^sender, System::EventArgs ^e) {
		...
		seriesDataGridView->Columns->Add(L"errColumn", GUICLR::UnicodeChars::chisqr);
		seriesDataGridView->Columns->Add(L"err2Column", GUICLR::UnicodeChars::rsqr);
	...
	void TWindow::wgt_MouseMove(System::Object ^sender, System::Windows::Forms::MouseEventArgs ^e) {
		...
		LocOnGraph->Text = "("+ Double(loc.first).ToString("0.000000") + ", "+ Double(loc.second).ToString("0.000000")+ ")";
